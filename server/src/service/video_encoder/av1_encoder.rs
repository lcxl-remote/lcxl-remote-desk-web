...
impl Av1Encoder {
    pub fn new(
        ...
        let ctx: Context<u8> = cfg.new_context().map_err(|e| {
            DeskError::AnyhowError(anyhow::anyhow!("rav1e context creation failed: {:?}", e))
        })?;
...
impl VideoEncoder for Av1Encoder {
    fn encode(&mut self, image_info: &dyn ImageInfo) -> Result<Vec<NalInfo>, DeskError> {
        ...
        self.ctx.send_frame(Arc::new(frame)).map_err(|e| {
            DeskError::AnyhowError(anyhow::anyhow!("rav1e send_frame failed: {:?}", e))
        })?;
        ...
        loop {
            match self.ctx.receive_packet() {
                ...
                Err(e) => {
                    return Err(DeskError::AnyhowError(anyhow::anyhow!(
