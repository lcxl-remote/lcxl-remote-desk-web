...
pub fn get_zbus_connection() -> Result<&'static ZBusConnection, DeskError> {
    ZBUS_CONNECTION
        .as_ref()
        .map_err(|err| DeskError::ZbusError(err.clone()))
}

pub fn get_zbus_portal_request(
    conn: &ZBusConnection,
    handle_token: &str,
) -> Result<Proxy<'static>, DeskError> {
    ...
    T: for<'de> Deserialize<'de> + Type,
{
    ...
    Err(DeskError::ZbusError(zbus::Error::Failure(format!(
