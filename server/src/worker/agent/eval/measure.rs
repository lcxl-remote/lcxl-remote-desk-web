...
//! serialized JSON size of its outcome. The numbers calibrate the diagnose
//! loop's context budget and latency thresholds. This is a measurement harness,
//! not an assertion suite: the [`measure_full_collection`] test is `#[ignore]`
//! and prints a table when run with `--nocapture`.
