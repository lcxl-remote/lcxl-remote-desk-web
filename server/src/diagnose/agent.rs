...
impl SessionSeam for InMemorySessionSeam {
    async fn claim_turn(
        ...
        // blocks and a same-conversation follow-up can continue its history. Direct
        // has no durable work item, so the verdict is conservatively
        // `InterruptedUnknown` (no fabricated execution identity); `begin_turn`
        // below rotates the lease token, fencing out the dead owner's late saves.
