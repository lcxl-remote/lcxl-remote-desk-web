...
//!
//! The model is asked for a bare JSON object (MVP spec §8), but real models wrap
//! it in prose or ```json fences. The parser extracts the outermost JSON object
//! and deserializes it; the diagnose DTOs already match the requested schema, so
//! a well-formed response maps directly. A response that cannot be parsed
