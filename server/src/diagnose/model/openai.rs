...
impl ModelAdapter for OpenAiCompatAdapter {
    async fn stream_chat(
        ...
        if !response.status().is_success() {
            ...
            // Read the gateway's error body (bounded) so the precise reason — a
            // bad model name, an unsupported `response_format`, an auth failure —
            // is visible instead of a bare status. The body is the gateway's own
            ...
            log::warn!("openai-compat gateway returned {status}: {detail}");
