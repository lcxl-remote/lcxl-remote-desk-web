//! Wire types for the in-Terminal AI Assistant.
//!
//! The assistant is a manager-owned AI control frame, structurally a sibling of
//! [`crate::diagnose`]: a control end sends a [`TerminalAiAssistantAsk`] and the
//! orchestrator streams back notification-style [`TerminalAiAssistantEvent`] frames
...
pub enum TerminalAiAssistantMode {
...
/// One prior exchange in a multi-turn assistant conversation, supplied by the
...
pub struct AssistantHistoryTurn {
...
pub struct TerminalAiAssistantAsk {
    ...
    pub mode: TerminalAiAssistantMode,
...
/// One command the assistant proposes. `risk` / `decision` are server-computed and
...
/// The assistant's structured answer (delivered in the terminal `Final` frame).
#[derive(
    Debug, Clone, PartialEq, Eq, Default, Serialize, Deserialize, SchemaWrite, SchemaRead, ToSchema,
)]
pub struct TerminalAiAssistantAnswer {
...
/// Discriminant for a [`TerminalAiAssistantEvent`] frame.
#[derive(
    Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, SchemaWrite, SchemaRead, ToSchema,
)]
#[serde(rename_all = "snake_case")]
pub enum TerminalAiAssistantEventKind {
...
/// One streamed frame of a assistant turn (server → control end). Notification
...
pub struct TerminalAiAssistantEvent {
    /// Correlates back to the originating `TerminalAiAssistantAsk` request.
    pub request_id: String,
    /// Monotonic per-stream sequence number.
    pub seq: u32,
    pub kind: TerminalAiAssistantEventKind,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub answer: Option<TerminalAiAssistantAnswer>,
...
impl TerminalAiAssistantEvent {
    ...
    pub fn partial(request_id: impl Into<String>, seq: u32, fragment: impl Into<String>) -> Self {
        Self {
            ...
            ..Self::base(request_id, seq, TerminalAiAssistantEventKind::Partial)
        }
    }
    ...
    pub fn retracted(
        ...
        Self {
            ...
            error,
            ..Self::base(request_id, seq, TerminalAiAssistantEventKind::Retracted)
        }
    }
    ...
    pub fn tool_started(
        ...
        Self {
            ...
            ..Self::base(request_id, seq, TerminalAiAssistantEventKind::ToolStarted)
        }
    }
    ...
    pub fn final_answer(
        request_id: impl Into<String>,
        seq: u32,
        answer: TerminalAiAssistantAnswer,
    ) -> Self {
        Self {
            answer: Some(answer),
            ..Self::base(request_id, seq, TerminalAiAssistantEventKind::Final)
        }
    }
    ...
    pub fn error(request_id: impl Into<String>, seq: u32, error: AgentError) -> Self {
        Self {
            error: Some(error),
            ..Self::base(request_id, seq, TerminalAiAssistantEventKind::Error)
        }
    }
    ...
    pub fn is_terminal(&self) -> bool {
        matches!(
            ...
            TerminalAiAssistantEventKind::Final
                ...
                | TerminalAiAssistantEventKind::Retracted
        )
    }
}
...
mod tests {
    ...
    fn sample_answer() -> TerminalAiAssistantAnswer {
    ...
    fn final_error_and_retracted_are_terminal() {
        assert!(!TerminalAiAssistantEvent::partial("r", 0, "x").is_terminal());
        assert!(!TerminalAiAssistantEvent::tool_started("r", 1, "read_process_list").is_terminal());
        assert!(TerminalAiAssistantEvent::final_answer("r", 2, sample_answer()).is_terminal());
        assert!(TerminalAiAssistantEvent::error("r", 3, sample_error()).is_terminal());
        assert!(!TerminalAiAssistantEvent::partial_committed("r", 2).is_terminal());
        let retracted = TerminalAiAssistantEvent::retracted(
            "r",
            3,
        ...
        let decoded: TerminalAiAssistantEvent =
    ...
    fn constructors_set_only_their_payload() {
        let p = TerminalAiAssistantEvent::partial("r", 0, "frag");
        assert_eq!(p.partial_text.as_deref(), Some("frag"));
        assert!(p.tool_name.is_none() && p.answer.is_none() && p.error.is_none());

        let f = TerminalAiAssistantEvent::final_answer("r", 1, sample_answer());
    ...
    fn event_wire_round_trips() {
        let cfg = unbounded_config();
        let original = TerminalAiAssistantEvent::final_answer("req-1", 7, sample_answer());
    ...
    fn final_frame_carries_provenance_and_round_trips() {
        ...
        let original =
            TerminalAiAssistantEvent::final_answer("req-1", 7, sample_answer()).with_provenance(
        ...
        let back: TerminalAiAssistantEvent = serde_json::from_str(&json).expect("json decode");
    ...
    fn ask_wire_round_trips() {
        let cfg = unbounded_config();
        let original = TerminalAiAssistantAsk {
            conversation_id: Some("c-1".into()),
            mode: TerminalAiAssistantMode::ExplainError,
            question: None,
        ...
        let bytes = wincode::config::serialize(&original, cfg).expect("wincode encode");
        let decoded: TerminalAiAssistantAsk =
            wincode::config::deserialize(&bytes, cfg).expect("wincode decode");
        assert_eq!(decoded, original);
    }
