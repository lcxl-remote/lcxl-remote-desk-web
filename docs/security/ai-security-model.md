...
Background promotion requires the original acceptance and frozen execution policy after the foreground budget but before the original hard deadline. It stores metadata on the same outbox, not a second executable work or renewed grant. The owner snapshot reads session, grants and old/new task projections in one SQLite transaction; its presentation cache is independent of execution CAS. Recovery correlates the original action instead of treating a missing legacy command row as proof of non-execution. A pending or corrupt page cannot starve all later receipts in the completion scan. Actual cancellation of these OSS task projections remains unconnected and `supports_cancel=false`.
