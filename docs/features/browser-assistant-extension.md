...
The LCXL Chrome extension is the default browser adapter for Device Assistant. It runs in Chrome on the controlled device and connects to an authenticated loopback bridge. Chrome DevTools MCP remains available only as a default-disabled development adapter; LCXL never falls back to it automatically.
...
2. In the local OSS Device Assistant page, choose **Show pairing code**. This owner-only response is marked `no-store`.
