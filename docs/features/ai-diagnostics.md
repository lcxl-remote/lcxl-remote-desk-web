...
An execution backend may also supply a verified original result label. The shared loop checks that it matches the result bytes and preserves it without regenerating lineage or extending expiry at delivery time. A mismatch rejects the result without acknowledging delivery. The label grants no implicit model export permission. Open-source Signal now persists original completion receipts for its bound, side-effecting Computer Action paths, including late results received after the foreground waiter exits. Foreground execution and active waiting use the same validated result projection. Legacy command results and inline browser reads retain their existing label paths; this does not yet enable the complete generic background-task publishing and cancellation workflow.
