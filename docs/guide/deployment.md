...
- **`LRD_BRAVE_SEARCH_API_KEY`** — optional server-side Brave Web Search credential. Device Assistant marks `search_public_web` available only when this value is present and valid; otherwise that capability is explicitly unavailable. The query must still appear verbatim in the current user message and consume one exact ExportData grant. The credential is never sent to the model, browser control end, controlled device, tool output, or Debug text.
