...
控制端可以在连接框输入访问码，连接在线被控端（`POST /api/desk/redeem-code`）。访问码可以是长期设备码，也可以是有时效、按需签发的一次性协助码。两者都连接同一个被控端，不存在单独的协助连接通道。
...
连接 Manager 的被控端每 30 秒发送一次应用层心跳。只有令牌和设备所有者快照仍然有效时，Manager 才会返回与请求编号关联的凭据证明。被控端使用单调时钟维护 120 秒凭据租约；开源、本地或远程信令返回的普通心跳不能延长该租约。
