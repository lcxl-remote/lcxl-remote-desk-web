...
- **per-code 能力上限随会话携带。**owner 可为每个码配置三态上限（允许 / 询问 / 拒绝），覆盖远程控制、剪贴板、私有屏、白板、终端、文件浏览、文件删除、文件传输。删除必须让浏览与删除两个维度同时通过；仅允许浏览时只能列目录和查看元数据。未配置的码默认全询问，绝不默认全权。详见[访问码](/zh/guide/access-codes)。
