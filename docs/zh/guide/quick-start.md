...
访问 `http://localhost:8081`。首次访问时会有一个分三步的设置向导：①创建管理员账户并同意协议；②可选地将被控端指向 manager（填入其域名——向导会解析 `wss`/`ws` 并校验 API 令牌，若 manager 仅提供明文 `ws`/`http` 而非加密连接，向导会给出安全提示但不会阻断——或跳过此步）；③选择入站安全开关与遥测选项，然后完成。
