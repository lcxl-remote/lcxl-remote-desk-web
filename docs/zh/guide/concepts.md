...
除了浏览器，AI 模型也可以读取并分析设备状态。会话内诊断严格按照**采集 → 脱敏 → 模型 → 展示**的顺序执行。在设备所有者自己的设备上，AI 助手可以提出命令请求，但只有所有者确认完整命令后，被控端才会执行；MCP 服务始终保持只读。见 [AI 诊断](/zh/features/ai-diagnostics)与 [AI 安全模型](/zh/security/ai-security-model)。
