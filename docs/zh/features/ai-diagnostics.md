...
**模型提供方**（供应商、base URL、模型、API Key、输出格式、授予的执行模式、单回合推理轮次上限以及单回合同一工具调用上限）在**中心信令服务器**上配置。推理轮次上限默认 **20**、取值范围 **1–50**，统计的是模型响应轮次而不是单个工具调用，并且不得小于同一工具调用上限。同一工具调用上限默认 **10**、取值范围 **1–50**；即使参数不同，只要工具类型相同也计入。两者都与 Desk 服务端的同时运行命令进程数相互独立。**API Key 是严格的服务端密钥**——绝不回传浏览器、绝不写日志、绝不进任何公开设置 DTO。
