...
Signal 权限决定重试在当前调用者和在线目标校验通过后返回首次决定，即使当前 readiness 不可用、授权已过期/撤销或原输入已被新问题替代也一样。完整决定的各项内容和限制必须与首次一致，条目顺序可以不同。重试不新签授权、不恢复次数、不改会话，也不另起模型续跑；原请求或决定记录缺失/损坏时，不能从当前批准状态反推回执。响应 `state` 是历史决定回执，不是当前权限或已续跑的证明。Signal 决定触发的持久续跑仍在实施，回执重放通过不代表重启恢复已完成。
