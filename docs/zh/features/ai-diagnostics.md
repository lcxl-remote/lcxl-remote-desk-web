...
执行后端也可提供已经验证的原始结果标签。共享循环验证结果字节与标签匹配后原样保留，不在投递时重新生成来源或延长有效期；不匹配时拒绝使用结果且不确认投递已消费。该标签不隐含模型外发授权。开源 Signal 现在为已绑定的副作用 Computer Action 保存原始完成回执，包括前台等待者退出后收到的迟到结果；前台执行与主动等待使用同一套经过校验的结果投影。旧命令结果与内联浏览器读取保留既有标签路径；这尚未开放完整通用后台任务发布和取消流程。
