...
pub fn serialize(output: &OperationOutput) -> Result<String, serde_json::Error> {
    ...
    if ui.nodes.is_empty() {
        body["search_hint"] = json!(
            "No results matched this bounded query. If your root is a macOS DesktopSession, this searched running application names only, not controls: when truncated=false and known localized/English application names have no match, open the requested application through an authorized launch tool, then search the session again. Do not increase UI depth or search button/date labels to find a non-running app. A truncated listing or read error does not prove absence. If your root is an application/window/control, this does not establish that the UI or operation is unsupported. Check the root and search task-specific localized/English labels or observed native_id (up to 16 OR alternatives). Locate the target window and any relevant dialog/popover/editor, then search within that root; use the window if no separate container exists. Only after targeted misses try control types such as AXTextField/input. Broad text/date/time terms can match unrelated calendar text; reserve them for fallback within a small region. Use allow_unfiltered=true only explicitly when targeted searches are insufficient."
        );
    }
    if ui.truncated {
        body["truncation_hint"] = json!(
            "This bounded UI tree is incomplete; do not conclude that the display/result is unreadable or the capability unsupported. First narrow root_id to the relevant observed dialog/editor/container if available; otherwise keep the target window. Search task-specific localized/English labels or native_id, then increase max_depth/max_nodes/max_bytes as needed to complete that bounded search. Do not add broad text/date/time alternatives by default or use element_only=true to search children. Read known controls with element_id and element_only=true. If still insufficient, inspect an authorized window screenshot to locate the target. Continue under active grants; no additional conversational confirmation is needed."
        );
    }
