...
pub fn is_requestable_desktop_read(name: &str) -> bool {
    matches!(
        name,
        "inspect_desktop_session" | "inspect_desktop_ui" | "read_current_screen"
    )
}
...
fn execute_confirmed_ui_action_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            ...
            description: "Execute exactly one bounded semantic action against a fresh UI element reference from inspect_desktop_ui. Authority must be an active application_scope grant. Request application_scope after observing the application, with the needed actions; each call then supplies that approved application reference plus a fresh target. The edge re-locates the element by its signed fingerprint, rechecks the target application allowlist and local ceiling, and reports whether the native UI API completed successfully. The permission request automatically includes separately reviewable desktop session and UI reads, excluding screenshots; check current grants after approval. Success does not verify the intended application state: call inspect_desktop_ui afterward, then decide the next authorized action. Never blindly repeat an action with an unknown outcome.".into(),
            parameters_schema: json!({
                "type": "object",
                "properties": {
...
fn prompt(locale: Option<&str>) -> String {
    ...
    text.push_str("\nKeep progress explanations brief: at most one or two sentences before a tool call. Do not repeat object tokens, approved arguments, or complete action batches in prose; the owner can review tool details and permission cards. For GUI tasks, inspect the application once, then read known result elements with element_only=true or query by native_id/role/name. Re-read a full UI tree only when precise lookup fails or the structure changed. Never replace a user-requested GUI workflow with shell calculation. Identify requested permissions by tool_name; the server derives the Provider and effect.");
...
mod tests {
    ...
    fn application_reference_is_required_only_for_native_ui_execution() {
        let action = execute_confirmed_ui_action_tool();
        assert_eq!(
    ...
    fn prompt_distinguishes_history_from_current_state_and_authority() {
        let text = build_device_assistant_system_message(None).text;
