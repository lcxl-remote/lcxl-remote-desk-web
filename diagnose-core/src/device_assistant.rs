...
fn prompt(locale: Option<&str>) -> String {
    ...
    text.push_str("\nKeep progress explanations brief: at most one or two sentences before a tool call. Empty or truncated UI searches do not prove a result is unreadable: correct self-only versus descendant search and increase insufficient depth before concluding unsupported. Recheck current grants before claiming expiry; when permission is missing, submit request_capability_grants directly instead of asking permission to request permission. Do not repeat object tokens, approved arguments, or complete action batches in prose; the owner can review tool details and permission cards. For GUI tasks, inspect the application once, then read known result elements with element_only=true or query by native_id/role/name. Re-read a full UI tree only when precise lookup fails or the structure changed. Never replace a user-requested GUI workflow with shell calculation. Identify requested permissions by tool_name; the server derives the Provider and effect.");
