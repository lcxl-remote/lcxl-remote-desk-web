...
use crate::read_tools::{ai_assistant_read_tool_registry, read_tool_registry};
use crate::registry::{RegisteredTool, ToolEffect};

pub const PREVIEW_COMPUTER_ACTION_TOOL: &str = "preview_computer_action";
pub const EXECUTE_BACKGROUND_INPUT_TOOL: &str = "send_background_input";
pub const EXECUTE_CONFIRMED_UI_ACTION_TOOL: &str = "execute_ui_actions";
pub const EXECUTE_CONFIRMED_RAW_INPUT_TOOL: &str = "send_raw_input";

pub const DESKTOP_SESSION_PROVIDER_ID: &str = "desktop.session";
pub const APPLICATION_CATALOG_PROVIDER_ID: &str = "application.catalog";
pub const APPLICATION_LIST_CAPABILITY_ID: &str = "application.list";
pub const APPLICATION_CATALOG_ADAPTER_ID: &str = "native-application-catalog";
pub const APPLICATION_CATALOG_ADAPTER_VERSION: &str = "native-application-catalog/v1";
...
/// validated Computer Use readiness report. AI Assistant treats this as an
...
pub const BROWSER_EXTENSION_ADAPTER_ID: &str = "browser.extension.edge";
...
pub const BROWSER_EXTENSION_ADAPTER_VERSION: &str = "lcxl-browser-extension/1";
...
/// AI Assistant control end. Inventory consumers use this server-authored
...
pub fn selected_context_capabilities(
    ...
    let mut capabilities = selected_capability_ids
        ...
        .map(|capability_id| match capability_id.as_str() {
            ...
            _ => Err(format!(
                "unknown or non-context AI Assistant capability: {capability_id}"
            )),
        })
        ...
        let provider_identities: &[(&str, &str, &str, &str)] = match entry.capability {
            Capability::BrowserPageObserve => &[
                (
                    ...
                    BROWSER_EXTENSION_ADAPTER_VERSION,
                ),
                (
...
fn spreadsheet_batch_patch_tool() -> RegisteredTool {
    ...
    tool.spec.name = "patch_numbers_copy".into();
    tool.spec.description = "Apply one exact typed value or formula patch to a fresh cell reference returned by inspect_numbers_file, then create a new .numbers copy in an exact owner-selected directory. The selected source is never overwritten; a private XLSX export is verified and deleted before publication.".into();
...
fn document_batch_patch_tool() -> RegisteredTool {
    ...
    tool.spec.name = "replace_pages_copy_body".into();
    tool.spec.description = "Replace the bounded body text of a fresh document reference returned by inspect_pages_file, then create a new .pages copy in an exact owner-selected directory. The selected source is never overwritten; a private PDF export is verified and deleted before publication.".into();
...
fn presentation_batch_patch_tool() -> RegisteredTool {
    ...
    tool.spec.name = "patch_keynote_copy".into();
    tool.spec.description = "Apply one exact title or presenter-notes patch to a fresh slide reference returned by inspect_keynote_file, then create a new .key copy in an exact owner-selected directory. The selected source is never overwritten; a private PDF export is verified and deleted before publication.".into();
...
/// AI Assistant tool registry: bounded host-side reads plus one central-only
...
fn execute_ui_actions_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            name: EXECUTE_CONFIRMED_UI_ACTION_TOOL.into(),
            description: "Execute 1–20 ordered semantic UI steps in one application using application_id and steps with observed element_id/action pairs. Reuse an active application_scope grant covering all action kinds; one batch consumes one use. Prefer semantic UI over background input. Stop at the first failure, do not roll back or retry automatically, and read UI afterward to verify application state. Split asynchronous UI dependencies into separate batches with a read between them.".into(),
            parameters_schema: json!({
                "type": "object",
                "properties": {
                    "application": {"type":"object", "description":"For an active application_scope grant only: copy its complete application reference from CURRENT AUTHORIZED GRANTS. Do not substitute the target UI reference or another application.", "properties":{"token":{"type":"string"},"snapshot_id":{"type":"string"},"object_kind":{"const":"application"},"expires_at":{"type":"string"}},"required":["token","snapshot_id","object_kind","expires_at"],"additionalProperties":false},
                    "target": {
                        "type": "object",
                        "properties": {
                            "token": {"type": "string", "minLength": 1},
                    ...
                    "action": {
                        "oneOf": [
                            ...
                            },
                            {
                                "type": "object",
                                "properties": {
                                    "kind": {"type": "string", "const": "toggle"},
                                    "params": {"type":"object","properties":{"desired":{"type":"boolean"}},"required":["desired"],"additionalProperties":false}
                                },
                                "required": ["kind", "params"],
                                "additionalProperties": false
                            },
                ...
                "required": ["application", "target", "action"],
                "additionalProperties": false
            }),
        },
...
fn send_raw_input_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
...
fn create_local_message_draft_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            name: "create_local_message_draft".into(),
...
fn send_gmail_message_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            ...
            parameters_schema: json!({
                ...
                "properties": {
                    ...
                    "handoff": {
                        "type": "object",
                        "description": "Copy the complete CommunicationDraftHandoff returned by prepare_gmail_draft without changes."
                    },
                    "page": browser_page_schema(),
                    "to_field": browser_element_schema(),
                    "subject_field": browser_element_schema(),
...
fn send_slack_message_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            ...
            parameters_schema: json!({
                ...
                "properties": {
                    ...
                    "handoff": {
                        "type": "object",
                        "description": "Copy the complete CommunicationDraftHandoff returned by prepare_slack_message without changes."
                    },
                    "page": browser_page_schema(),
                    "composer": browser_element_schema(),
                    "send_control": browser_element_schema(),
...
fn browser_origin_schema() -> serde_json::Value {
    json!({
        "type": "object",
        "properties": {
            "kind": {"type": "string", "enum": ["https", "http_loopback", "http"]},
...
fn browser_open_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            ...
            description: "Open one provider-owned Chrome page at an exact HTTP or HTTPS URL. This is a browser application mutation and never exposes arbitrary tab inventory, cookies, storage, network logs, or raw DOM.".into(),
            parameters_schema: json!({
                "type": "object",
                "properties": {
                    "target": {
...
fn create_spreadsheet_formula_artifact_tool() -> RegisteredTool {
    RegisteredTool {
        spec: ToolSpec {
            name: "create_formula_workbook".into(),
...
pub fn ai_assistant_provider_registry() -> ProviderRegistry {
        ...
        .filter(|tool| tool.name() != "read_current_screen")
    {
        assert!(
            ...
            "duplicate AI Assistant read tool"
        );
    }
    ...
    let mut application_launch = provider_for_tool(
        ...
        vec![crate::application_launch::ADAPTER_ID.into()],
        ExecutionLocality::Edge,
        CapabilityEffect::LaunchApplication,
        1,
        Vec::new(),
        vec![CapabilityDataCategory::UserRequest],
        vec![AuthorizationResourceKind::ExactApplication],
        crate::application_launch::tool(),
    );
    for capability in &mut application_launch.wire.capabilities {
        capability
            .prerequisites
            .platforms
            .push(CapabilityPlatform::Linux);
    }
    for capability in &mut application_launch.capabilities {
        capability
            .wire
            .prerequisites
            .platforms
    ...
    let mut application_catalog = provider_for_tool(
        ...
        vec![APPLICATION_CATALOG_ADAPTER_ID.into()],
        ExecutionLocality::Edge,
        CapabilityEffect::ReadDevice,
        100,
        Vec::new(),
        vec![
            ...
            CapabilityDataCategory::ProcessMetadata,
        ],
        vec![AuthorizationResourceKind::TargetDevice],
        reads
            .remove("list_applications")
            .expect("static application catalog tool exists"),
    );
    for capability in &mut application_catalog.wire.capabilities {
        capability
            .prerequisites
            .platforms
            .push(CapabilityPlatform::Linux);
    }
    for capability in &mut application_catalog.capabilities {
        capability
            .wire
            .prerequisites
            .platforms
    ...
    let browser_open = provider_for_tool(
        ...
        vec![BROWSER_EXTENSION_ADAPTER_ID.into()],
        ExecutionLocality::Edge,
        CapabilityEffect::MutateApplication,
        1,
        Vec::new(),
...
/// inventories.
pub fn ai_assistant_edge_adapter_registry() -> EdgeAdapterRegistry {
    ...
    EdgeAdapterRegistryBuilder::new()
        .register(adapter(
            crate::application_launch::ADAPTER_ID,
            crate::application_launch::ADAPTER_VERSION,
            crate::application_launch::CAPABILITY_ID,
        ))
        .register(adapter(
            APPLICATION_CATALOG_ADAPTER_ID,
            APPLICATION_CATALOG_ADAPTER_VERSION,
            APPLICATION_LIST_CAPABILITY_ID,
        ))
        ...
        .register(EdgeAdapterDescriptor {
            adapter_id: BROWSER_EXTENSION_ADAPTER_ID.into(),
            adapter_version: BROWSER_EXTENSION_ADAPTER_VERSION.into(),
            capability_ids: vec![
        ...
        .build(&providers)
        .expect("static AI Assistant edge adapter registry must be valid")
}

/// Agent-loop projection generated exclusively from the Provider Registry.
pub fn ai_assistant_tool_registry() -> Vec<RegisteredTool> {
    ai_assistant_provider_registry().registered_tools()
}
...
pub fn validate_preview_call(call: &ToolCall) -> Result<String, AgentError> {
    if call.name != PREVIEW_COMPUTER_ACTION_TOOL {
        return Err(AgentError {
            ...
            message: "not a AI Assistant preview tool".into(),
            retryable: false,
            safe_for_model: true,
            error_code: None,
        });
...
pub fn current_time_prompt(now_unix_ms: u64) -> String {
    ...
    format!(
        "\nCURRENT TIME (server clock): {}. This is UTC, not the user's or target device's local timezone. Never use a remembered/model-training date for today, tomorrow or the day after tomorrow. For a native Calendar UI task, establish the current local date/time and offset from current device/application evidence or the user's explicit instructions; a selected event date is not today's date. Do not infer timezone from language or the server timezone. Ask only when the required local date/time remains ambiguous. Scheduling-tool timezone requirements remain separate.\n",
...
mod tests {
    ...
    #[test]
    fn public_search_schema_retains_its_natural_language_query_contract() {
        let tool = search_public_web_tool();
    ...
    #[test]
    fn application_reference_is_required_only_for_native_ui_execution() {
        let action = execute_ui_actions_tool();
        assert_eq!(
            action.spec.parameters_schema["required"],
            serde_json::json!(["application", "target", "action"])
        );
        let kinds = action.spec.parameters_schema["properties"]["action"]["oneOf"]
            .as_array()
            .unwrap()
            .iter()
        ...
        let reads = crate::read_tools::ai_assistant_read_tool_registry();
        let read = reads
            .iter()
            .find(|tool| tool.name() == "inspect_desktop_ui")
            .unwrap();
        let required = read
            .spec
            .parameters_schema
        ...
        assert!(
            required.is_none_or(|fields| !fields.iter().any(|v| v == "root" || v == "application"))
        );
        assert_eq!(
            spreadsheet_live_patch_tool().spec.parameters_schema["required"],
            serde_json::json!(["target", "action"])
        );
    }

    #[test]
    fn desktop_reads_are_planning_candidates_without_implicit_authority() {
        let providers = ai_assistant_provider_registry();
        let mut tools = ai_assistant_tool_registry();
        retain_selected_context_tools(&providers, &mut tools, &[]);
        let scope = selected_context_capabilities(&[]).unwrap();
        for name in [
            ...
            assert!(!scope.contains(&tool.required_capability));
        ...
        assert!(!tools.iter().any(|t| t.name() == "inspect_office_selection"));
    }

    #[test]
    fn browser_tool_schema_targets_only_the_chrome_extension() {
        let schema = browser_adapter_schema();
        assert_eq!(schema["properties"]["engine"]["const"], "chrome_extension");
        assert_eq!(
            schema["properties"]["browser_major_version"]["minimum"],
            120
        );
    ...
    #[test]
    fn gui_guidance_uses_bilingual_search_and_native_lifetimes() {
        let text = build_ai_assistant_system_message(Some("zh-CN")).text;
        assert!(text.contains("task-specific localized/English labels"));
        assert!(text.contains("Search in stages:"));
        assert!(text.contains("Only after targeted misses add control types"));
        assert!(text.contains("invoke it yourself"));
        assert!(text.contains("empty owner_selectable_windows list is not evidence"));
        assert!(text.contains("quick-entry field is not the title field"));
        assert!(!text.contains("omit an expired root"));
        assert!(!text.contains("reconstruct a missing object_ref.token"));
        assert!(!text.contains("Never claim a change occurred."));
    }

    #[test]
    fn current_time_context_is_utc_and_does_not_invent_local_timezone() {
        let text = current_time_prompt(1789125115000);
        assert!(text.contains("2026-09-11T11:11:55Z"));
        assert!(text.contains("not the user's or target device's local timezone"));
        assert!(text.contains("selected event date is not today's date"));
        assert!(current_time_prompt(u64::MAX).is_empty());
    }
    ...
    fn prompt_distinguishes_history_from_current_state_and_authority() {
        ...
        assert!(text.contains("do not present an old observation as current state"));
        assert!(text.contains("never renew an expired grant"));
    }

    #[test]
    fn prompt_prioritizes_newest_followup_and_bars_projection_loops() {
        let prompt = build_ai_assistant_system_message(None).text;
    ...
    #[test]
    fn edge_inventory_covers_every_non_central_ai_assistant_capability() {
        let providers = ai_assistant_provider_registry();
        let edge = ai_assistant_edge_adapter_registry();
        let registered = edge
            .adapters()
    ...
    fn provider_projection_matches_the_explicit_static_tool_set() {
        let mut legacy = ai_assistant_read_tool_registry();
        legacy.extend(
            read_tool_registry()
                .into_iter()
                .filter(|tool| tool.name() != "read_current_screen"),
        ...
        legacy.push(create_local_message_draft_tool());
        ...
        legacy.push(send_gmail_message_tool());
        legacy.push(send_slack_message_tool());
        ...
        legacy.push(exec_command_tool());
        legacy.push(crate::application_launch::tool());
        ...
        let projected = ai_assistant_tool_registry();
    ...
    fn context_selection_keeps_system_tools_and_is_exact_for_selected_context() {
        let providers = ai_assistant_provider_registry();
        let mut empty = ai_assistant_tool_registry();
    ...
    fn prompt_is_explicit_about_the_bounded_artifact_mutations() {
        ...
        let message = build_ai_assistant_system_message(Some("zh-CN"));
        assert!(
            message
                .text
        ...
        assert!(message.text.contains("exec_command"));
        assert!(message.text.contains("R3 one-shot exact grant"));
        assert!(message.text.contains("create_workbook"));
        assert!(message.text.contains("create_local_message_draft"));
        ...
        assert!(message.text.contains("create_word_report"));
