...
#[test]
fn application_scope_is_reusable_but_cannot_cross_actions_apps_or_expiry() {
    ...
    let decisions = vec![PermissionDecisionItem {
        ...
        decision: PermissionItemDecision::Approve {
            resource_scope: item.resource_scope.clone(),
            operation_scope: vec!["ui:set_value".into()],
            export_destinations: vec![],
            ttl_seconds: 120,
            max_uses: 8,
        },
    ...
    for surface in [
        ...
        let preflight =
            UiCallPreflight::build(&registry, surface, &call(&input), now + 1000).unwrap();
