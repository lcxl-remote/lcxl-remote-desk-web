//! Terminal AI Assistant: model-agnostic prompt assembly, response parsing, and
...
use desk_agent_protocol::terminal_ai_assistant::{
    AssistantHistoryTurn, CommandSuggestion, TerminalAiAssistantAnswer, TerminalAiAssistantAsk,
    TerminalAiAssistantEvent, TerminalAiAssistantMode,
};
...
/// Default per-turn reasoning-round budget for the interactive assistant.
///
/// The manager treats this as the fallback when its platform-configured limit
/// (`ai.terminal_ai_assistant.max_steps_per_turn`) is absent or unparseable.
...
pub const MAX_AI_ASSISTANT_HISTORY_TURNS: usize = 6;
...
pub const MAX_AI_ASSISTANT_HISTORY_MSG_BYTES: usize = 2_000;
...
/// The read-only tools the assistant may call to gather evidence: a deliberately
...
/// run it.
pub fn assistant_read_tools() -> Vec<RegisteredTool> {
...
/// Build the assistant system prompt for `mode`. Not persisted in the conversation
...
pub fn build_assistant_system_message(
...
pub fn build_assistant_system_message_without_content_policy(
    ...
    let task = match mode {
        ...
        TerminalAiAssistantMode::ExplainError => {
    ...
    ChatMessage::text("assistant-system", ChatRole::System, body)
}
...
pub fn build_assistant_user_message(ask: &TerminalAiAssistantAsk) -> ChatMessage {
    let ctx = &ask.context;
    ...
    ChatMessage::text("assistant-user", ChatRole::User, body)
}

/// Build the prior-conversation messages for a stateless multi-turn assistant turn:
/// the last [`MAX_AI_ASSISTANT_HISTORY_TURNS`] turns mapped to alternating user /
/// assistant [`ChatMessage`]s, each length-capped to [`MAX_AI_ASSISTANT_HISTORY_MSG_BYTES`].
...
pub fn build_assistant_history_messages(history: &[AssistantHistoryTurn]) -> Vec<ChatMessage> {
    let start = history.len().saturating_sub(MAX_AI_ASSISTANT_HISTORY_TURNS);
    let mut messages = Vec::new();
    for turn in &history[start..] {
        let user = truncate_on_char_boundary(turn.user.trim(), MAX_AI_ASSISTANT_HISTORY_MSG_BYTES);
        if !user.is_empty() {
            messages.push(ChatMessage::text(
                "assistant-history-user",
                ChatRole::User,
                user,
            ));
        }
        let assistant =
            truncate_on_char_boundary(turn.assistant.trim(), MAX_AI_ASSISTANT_HISTORY_MSG_BYTES);
...
/// Parse the model's final answer into a [`TerminalAiAssistantAnswer`], computing the
...
/// Emits assembled [`TerminalAiAssistantEvent`] frames over a runtime's outbound
...
/// blanket impl covers any `Fn(TerminalAiAssistantEvent)`, so a runtime passes a
/// closure that forwards to its own sink.
pub trait AssistantFrameSink {
    fn emit(&self, event: TerminalAiAssistantEvent);
}

impl<F: Fn(TerminalAiAssistantEvent)> AssistantFrameSink for F {
    fn emit(&self, event: TerminalAiAssistantEvent) {
        self(event)
    }
}
...
/// first ``` code fence. The assistant prompt asks for the explanation as prose
...
/// [`TerminalAiAssistantEvent`] frames with a monotonic `seq`, forwarding each to a
/// [`AssistantFrameSink`]. Shared by both runtimes so they can never drift on frame
...
pub struct AssistantStreamSink<S> {
    sink: S,
    request_id: String,
    seq: u32,
    terminated: bool,
...
impl<S: AssistantFrameSink> AssistantStreamSink<S> {
    /// Build a sink streaming one assistant request's frames to `sink`. Text
    ...
    pub fn emit_final(&mut self, answer: TerminalAiAssistantAnswer) {
        if self.terminated {
            return;
        }
        let seq = self.next_seq();
        let mut frame =
            TerminalAiAssistantEvent::final_answer(self.request_id.clone(), seq, answer);
    ...
    fn emit_error_with_reason(
        ...
        let event = match reason {
            Some(reason) => TerminalAiAssistantEvent::error_with_retraction_reason(
                self.request_id.clone(),
                seq,
                error,
                reason,
            ),
            None => TerminalAiAssistantEvent::error(self.request_id.clone(), seq, error),
        };
...
impl<S: AssistantFrameSink> TurnSink for AssistantStreamSink<S> {
    fn on_text_delta(&mut self, delta: &str) {
        ...
        if safe > self.emitted {
            ...
            self.emitted = safe;
            let seq = self.next_seq();
            self.sink.emit(TerminalAiAssistantEvent::partial(
                self.request_id.clone(),
                seq,
                fragment,
            ));
    ...
    fn on_partial_committed(&mut self) {
        ...
        if self.uncommitted_partial {
            let seq = self.next_seq();
            self.sink.emit(TerminalAiAssistantEvent::partial_committed(
                self.request_id.clone(),
                seq,
            ));
        }
    ...
    fn on_turn_retracted(&mut self, reason: StreamRetractionReason, error: Option<AgentError>) {
        ...
        if self.uncommitted_partial {
            let seq = self.next_seq();
            self.sink.emit(TerminalAiAssistantEvent::retracted(
                self.request_id.clone(),
                seq,
                reason,
                error,
    ...
    fn on_tool_started(&mut self, tool_name: &str, _call_id: &str, _arguments_json: &str) {
        ...
        let seq = self.next_seq();
        self.sink.emit(TerminalAiAssistantEvent::tool_started(
            self.request_id.clone(),
            seq,
            tool_name,
        ));
...
mod tests {
    ...
    use desk_agent_protocol::terminal_ai_assistant::{
        TerminalAiAssistantEventKind, TerminalContext,
    };
    use std::cell::RefCell;
    use std::rc::Rc;

    fn ask(mode: TerminalAiAssistantMode) -> TerminalAiAssistantAsk {
    ...
    fn system_prompt_differs_by_mode_and_states_constraints() {
        let howto = build_assistant_system_message(TerminalAiAssistantMode::HowTo, None).text;
        let explain =
            build_assistant_system_message(TerminalAiAssistantMode::ExplainError, None).text;
    ...
    fn system_prompt_injects_locale_language_rule_when_present() {
        let neutral = build_assistant_system_message(TerminalAiAssistantMode::HowTo, None).text;
        assert!(!neutral.contains("UI language"));
        // A blank / whitespace locale is treated as absent.
        let blank = build_assistant_system_message(TerminalAiAssistantMode::HowTo, Some("  ")).text;
        assert!(!blank.contains("UI language"));
        // A real locale steers the answer language and is echoed into the prompt.
        let localized =
            build_assistant_system_message(TerminalAiAssistantMode::ExplainError, Some("zh-CN"))
                .text;
    ...
    fn user_message_caps_recent_output() {
        let mut a = ask(TerminalAiAssistantMode::HowTo);
        a.context.recent_output = "x".repeat(MAX_RECENT_OUTPUT_BYTES * 2);
        let msg = build_assistant_user_message(&a);
    ...
    fn history_messages_alternate_roles_in_order() {
        ...
        let msgs = build_assistant_history_messages(&history);
    ...
    fn history_keeps_only_the_last_n_turns() {
        let history: Vec<AssistantHistoryTurn> = (0..(MAX_AI_ASSISTANT_HISTORY_TURNS + 3))
            .map(|i| AssistantHistoryTurn {
                user: format!("u{i}"),
        ...
        assert_eq!(msgs.len(), MAX_AI_ASSISTANT_HISTORY_TURNS * 2);
    ...
    fn history_caps_message_bytes_and_skips_empty() {
        let history = vec![
            AssistantHistoryTurn {
                user: "x".repeat(MAX_AI_ASSISTANT_HISTORY_MSG_BYTES * 2),
                assistant: "   ".into(),
            },
            AssistantHistoryTurn {
                user: "  ".into(),
        ...
        assert!(msgs[0].text.len() <= MAX_AI_ASSISTANT_HISTORY_MSG_BYTES);
    ...
    fn read_tool_subset_is_exactly_two() {
        let names: Vec<String> = assistant_read_tools()
            .into_iter()
            .map(|t| t.spec.name)
            .collect();
    ...
    fn structured_answer_classifies_each_suggestion() {
        ...
        let (answer, outcome) = parse_assistant_answer(content, "bash");
    ...
    fn empty_command_suggestions_are_dropped() {
        ...
        let (answer, _) = parse_assistant_answer(content, "bash");
        assert!(answer.suggestions.is_empty());
    }

    #[test]
    fn malformed_json_degrades_to_explanation_only() {
        let (answer, outcome) = parse_assistant_answer("sorry, no JSON here", "bash");
    ...
    fn empty_fenced_block_degrades() {
        let (answer, outcome) = parse_assistant_answer("no suggestions\n```json\n```", "bash");
        assert_eq!(outcome, ParseOutcome::Degraded);
        assert!(answer.suggestions.is_empty());
    ...
    fn streaming_sink_forwards_prose_and_withholds_json_block() {
        ...
        let mut s = AssistantStreamSink::new(sink, "req-s").streaming_text();
        s.on_text_delta("Free the ");
        s.on_text_delta("port.\n");
        s.on_text_delta("```json\n");
        s.on_text_delta("{\"suggestions\": []}\n```");
        s.emit_final(TerminalAiAssistantAnswer {
        ...
        let partials: String = ev
            .iter()
            .filter(|e| e.kind == TerminalAiAssistantEventKind::Partial)
        ...
        assert_eq!(ev.last().unwrap().kind, TerminalAiAssistantEventKind::Final);
    }
    ...
    fn default_sink_drops_text_deltas() {
        let (store, sink) = recorder();
        let mut s = AssistantStreamSink::new(sink, "req-d");
    ...
    fn stream_sink_maps_tool_then_final_in_order() {
        let (store, sink) = recorder();
        let mut s = AssistantStreamSink::new(sink, "req-1");
        ...
        assert_eq!(
            kinds,
            vec![
                TerminalAiAssistantEventKind::ToolStarted,
                TerminalAiAssistantEventKind::Final,
            ]
        );
    ...
    fn stream_sink_stamps_injected_provenance_on_final() {
        let (store, sink) = recorder();
        let mut s = AssistantStreamSink::new(sink, "req-p");
    ...
    fn stream_sink_final_without_provenance_omits_it() {
        let (store, sink) = recorder();
        let mut s = AssistantStreamSink::new(sink, "req-n");
        s.emit_final(TerminalAiAssistantAnswer {
            explanation_md: "done".into(),
            suggestions: Vec::new(),
        });
    ...
    fn stream_sink_latches_after_terminal() {
        let (store, sink) = recorder();
        let mut s = AssistantStreamSink::new(sink, "r");
        s.emit_final(TerminalAiAssistantAnswer {
        ...
        let ev = store.borrow();
        assert_eq!(ev.len(), 1);
        assert_eq!(ev[0].kind, TerminalAiAssistantEventKind::Final);
    }
    ...
    fn stream_sink_retracts_provisional_text_once_and_ignores_late_delta() {
        ...
        let (store, sink) = recorder();
        let mut stream = AssistantStreamSink::new(sink, "r").streaming_text();
        ...
        assert_eq!(events[0].kind, TerminalAiAssistantEventKind::Partial);
        assert_eq!(events[1].kind, TerminalAiAssistantEventKind::Retracted);
    ...
    fn stream_sink_policy_failure_without_partial_is_reasoned_error() {
        ...
        assert_eq!(events[0].kind, TerminalAiAssistantEventKind::Error);
        assert_eq!(
    ...
    fn stream_sink_commits_before_tool_and_uses_error_after_commit() {
        ...
        assert_eq!(
            ...
            vec![
                TerminalAiAssistantEventKind::Partial,
                TerminalAiAssistantEventKind::PartialCommitted,
                TerminalAiAssistantEventKind::ToolStarted,
                TerminalAiAssistantEventKind::Error,
            ]
        );
