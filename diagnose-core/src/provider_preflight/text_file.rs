        ...
        RegisteredTool {
            spec: ToolSpec {
                ...
                description: if delete {
                    "Delete exactly one verified text file on the controlled device after saving its previous content in device-private backup storage. Requires a currently approved conversation directory and one exact user confirmation. Reuse a verified creation/read/update result and its full SHA-256; a separate read is not required just to obtain a version. Text files only (UTF-8, at most 64 KiB), not installers or arbitrary binary files. Never recursive, never permanent deletion. Preserve the recovery receipt; an unknown result must not be retried."
                } else {
                    "Update exactly one verified UTF-8 text file on the controlled device using full replacement or exactly one unambiguous text match, within 64 KiB. Requires a currently approved conversation directory and one exact user confirmation. Reuse a verified creation/read/update result: copy its file_result_call_id and full SHA-256. If the content needed for the edit is already known, request this update directly; do not list the directory or read again merely to obtain a version. The device checks the original identity and full SHA-256, retains recovery material and reports the filesystem operation result without rereading the written bytes. Conflict means read again and request new approval, never retry automatically."
                }.into(),
