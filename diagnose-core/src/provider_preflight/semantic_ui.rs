...
fn unavailable() -> AgentError {
    error(
        ...
        "semantic UI input or original object is unavailable",
        false,
        true,
    )
}

pub fn ui_action_from_call(call: &ToolCall) -> Result<(ObjectRef, UiSemanticAction), AgentError> {
...
impl UiCallPreflight {
    pub fn build(
        ...
        if !matches!(
            ...
            || capability.required_capability != Capability::DesktopUiActionConfirmed
    ...
    pub fn action(&self) -> &UiSemanticAction {
        &self.action
    }
