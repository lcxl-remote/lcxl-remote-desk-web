...
export function LaunchConfirmationCard({ value }: { value: LaunchReview }) {
    ...
    return <div className="mt-3 space-y-2 rounded-md border p-3 text-xs" data-testid="launch-confirmation">
        <p className="font-semibold">{t('pages.aiAssistant.launchConfirmTitle')}</p>
        <p>{t('pages.aiAssistant.launchLifetime')}</p>
        <dl className="grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1">
            <dt>{t('pages.aiAssistant.launchTarget')}</dt><dd className="break-all">{value.target.value}</dd>
            <dt>{t('pages.aiAssistant.launchResolvedTarget')}</dt><dd className="break-all">{value.resolvedTarget}</dd>
            <dt>{t('pages.aiAssistant.commandCwd')}</dt><dd className="break-all">{value.cwd ?? t('pages.aiAssistant.launchDefaultCwd')}</dd>
            <dt>{t('pages.aiAssistant.commandTarget')}</dt><dd className="break-all">{value.targetDeviceId} / {value.targetSessionId}</dd>
            <dt>{t('pages.aiAssistant.launchPrivilege')}</dt><dd>{t(value.runAsAdmin ? 'pages.aiAssistant.launchAdmin' : 'pages.aiAssistant.launchUser')}</dd>
        </dl>
        {value.runAsAdmin && <p className="font-medium text-amber-700 dark:text-amber-300">{t('pages.aiAssistant.launchAdminWarning')}</p>}
        <p className="font-medium">{t('pages.aiAssistant.launchArgs')}</p>
