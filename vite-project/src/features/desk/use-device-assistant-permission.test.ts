...
beforeEach(() => {
    ...
    localStorage.setItem('device-assistant-conversation:desk-2', 'conversation-2');
});
...
describe('assistant permission submission fencing', () => {
    it.each(['committing', 'stale-input'])('does not approve %s requests', async state => {
        ...
        const { result } = renderHook(() => useDeviceAssistantChat({ deskId: 'desk-1', connected: true, subscribe, sendMessage }));
    ...
    it.each(['device', 'disconnect', 'reset'] as const)('ignores an old response after %s and blocks synchronous double submission', async change => {
        ...
        const { result, rerender } = renderHook(({ deskId, connected }) => useDeviceAssistantChat({ deskId, connected, subscribe, sendMessage }),
