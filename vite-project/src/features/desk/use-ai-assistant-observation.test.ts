...
describe('delayed observation', () => {
    ...
    function setup() {
        ...
        const hook = renderHook(({ deskId, enabled }) => useAiAssistantObservation({ deskId, enabled, subscribe, sendMessage }), {
...
describe('background application selection', () => {
    it.each(['macos', 'windows'])('fetches a fresh %s session then sends the selected opaque application root', (os) => {
        ...
        const { result, unmount } = renderHook(() => useAiAssistantObservation({ deskId: 'mac', subscribe, sendMessage }));
