...
describe('guided run workspace admission', () => {
    it('mounts the workspace only after a matching server record arrives', async () => {
        ...
        render(<AiAssistantRehearsalGate rehearsalId="run-1" deviceId="device">{workspace}</AiAssistantRehearsalGate>);
    ...
    it('unmounts the admitted workspace when its signaling client changes', async () => {
        const workspace = vi.fn(rehearsal => <p>{rehearsal.prompt}</p>);
        const view = render(<AiAssistantRehearsalGate rehearsalId="run-1" deviceId="device">{workspace}</AiAssistantRehearsalGate>);
        await act(async () => respond(0));
        expect(screen.getByText('Frozen task')).toBeInTheDocument();
        workspace.mockClear();
        transport.cancel = vi.fn();
        view.rerender(<AiAssistantRehearsalGate rehearsalId="run-1" deviceId="device">{workspace}</AiAssistantRehearsalGate>);
    ...
    it('ignores an earlier lookup after the launch parameter changes', async () => {
        const workspace = vi.fn(rehearsal => <p>{rehearsal.prompt}</p>);
        const view = render(<AiAssistantRehearsalGate rehearsalId="run-1" deviceId="device">{workspace}</AiAssistantRehearsalGate>);
        view.rerender(<AiAssistantRehearsalGate rehearsalId="run-2" deviceId="device">{workspace}</AiAssistantRehearsalGate>);
        await act(async () => respond(0));
        expect(workspace).not.toHaveBeenCalled();
