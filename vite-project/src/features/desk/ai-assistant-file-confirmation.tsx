...
export function TextFileConfirmationCard({ value }: { value: TextFileReview }) {
    ...
    const block = (title: string, text: string) => <div><p>{t(title)}</p>
        <pre className="max-h-48 overflow-auto whitespace-pre-wrap break-words rounded bg-muted p-2">{text || t('pages.aiAssistant.fileConfirmEmpty')}</pre></div>;
    return <div data-testid="text-file-confirmation" className="mt-3 max-w-full space-y-2 rounded-md border p-3 text-xs">
        <p className="font-semibold">{t(value.operation === 'delete' ? 'pages.aiAssistant.fileConfirmDelete' : 'pages.aiAssistant.fileConfirmUpdate')}: {value.fileName}</p>
        <p>{t('pages.aiAssistant.fileConfirmOneShot')}</p>
        {value.operation === 'delete' && <p>{t('pages.aiAssistant.fileConfirmRecoverable')}</p>}
        {value.change?.kind === 'replace_once' && <>{block('pages.aiAssistant.fileConfirmBefore', value.change.before!)}{block('pages.aiAssistant.fileConfirmAfter', value.change.after!)}</>}
        {value.change?.kind === 'replace_all' && block('pages.aiAssistant.fileConfirmReplaceAll', value.change.content_utf8!)}
