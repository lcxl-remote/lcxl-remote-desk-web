...
export function CommandConfirmationCard({ value }: { value: CommandReview }) {
    ...
    return (
        <div className="mt-3 space-y-2 rounded-md border p-3 text-xs" data-testid="command-confirmation">
            <p className="font-semibold">{t('pages.aiAssistant.commandConfirmTitle')}</p>
            <p>{t(value.executionBasis === 'owner_blocklist_only'
                ? 'pages.aiAssistant.commandFreeformWarning'
                : 'pages.aiAssistant.commandOneShot')}</p>
            <pre className="max-h-64 overflow-auto whitespace-pre-wrap break-words rounded bg-muted p-2">{value.command}</pre>
            <dl className="grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1">
                <dt>{t('pages.aiAssistant.commandShell')}</dt><dd>{value.shell}</dd>
                <dt>{t('pages.aiAssistant.commandCwd')}</dt><dd className="break-all">{value.cwd ?? t('pages.aiAssistant.commandDefaultCwd')}</dd>
                <dt>{t('pages.aiAssistant.commandTarget')}</dt><dd className="break-all">{value.targetDeviceId} / {value.targetSessionId}</dd>
                <dt>{t('pages.aiAssistant.commandLimits')}</dt><dd>{t('pages.aiAssistant.commandLimitsValue', {
