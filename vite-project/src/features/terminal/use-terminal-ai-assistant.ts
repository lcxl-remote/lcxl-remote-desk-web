...
import { v4 } from 'uuid';
import {
    SIGNALING_TYPE_CODE_ASK_TERMINAL_AI_ASSISTANT,
    SIGNALING_TYPE_CODE_TERMINAL_AI_ASSISTANT_UPDATED,
    SIGNALING_TYPE_CODE_CANCEL_TERMINAL_AI_ASSISTANT,
} from '../desk/constants';
...
// Wire types — mirror `desk_agent_protocol::terminal_ai_assistant`. They ride the
// `TerminalAiAssistant{Ask,Event,Cancel}` signaling types as `signaling_data`; like
// these signaling payloads are not part of the REST OpenAPI surface, so they
// are declared here.

export type TerminalAiAssistantMode = 'how_to' | 'explain_error';
...
export type TerminalAiAssistantAnswer = {
...
export type TerminalAiAssistantEventKind =
    | 'partial'
    | 'partial_committed'
    | 'retracted'
    | 'tool_started'
...
export type TerminalAiAssistantEvent = {
    request_id: string;
    seq: number;
    kind: TerminalAiAssistantEventKind;
    partial_text?: string | null;
    tool_name?: string | null;
    answer?: TerminalAiAssistantAnswer | null;
...
/** One read-only evidence tool the assistant dispatched, shown in the activity
 *  line. The assistant has no approval-gated tools, so there is no awaiting state. */
export type AssistantTool = {
    name: string;
};

export type AssistantPhase = 'idle' | 'running' | 'done' | 'error';
...
export type TerminalAiAssistantTurn = {
...
type UseTerminalAiAssistantProps = {
...
/**
 * Drives the in-Terminal AI Assistant over control-plane signaling: sends a
 * `TerminalAiAssistantAsk`, aggregates the notification-style `TerminalAiAssistantEvent`
 ...
 * deliberately separate: the assistant rides the desk signaling WS, the shell I/O
 * rides its own `/api/desk/terminal/{id}` WS.
 */
export function useTerminalAiAssistant({
    connectionId,
    subscribe,
    sendMessage,
}: UseTerminalAiAssistantProps) {
    ...
    const turnsRef = useRef<TerminalAiAssistantTurn[]>([]);
    useEffect(() => {
    ...
    const ask = useCallback(
        (input: AskInput) => {
            ...
            const history: AssistantHistoryTurn[] = turnsRef.current
                .filter((t) => t.answer)
    ...
    useEffect(() => {
        const handle = (message: SignalingMessage) => {
            if (message.signaling_type !== SIGNALING_TYPE_CODE_TERMINAL_AI_ASSISTANT_UPDATED) return;
            const event = message.signaling_data as TerminalAiAssistantEvent | null;
                ...
                switch (event.kind) {
                    ...
                    case 'error':
                        ...
                        return {
                            ...prev,
                            phase: 'error',
                            error: event.error?.message ?? 'assistant failed',
