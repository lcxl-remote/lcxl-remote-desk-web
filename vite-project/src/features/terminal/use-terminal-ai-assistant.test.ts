...
import { useTerminalAiAssistant } from './use-terminal-ai-assistant';
import type { TerminalAiAssistantEvent, TerminalContext } from './use-terminal-ai-assistant';
...
function eventFrame(event: TerminalAiAssistantEvent): SignalingMessage {
    return {
        request_id: event.request_id,
        signaling_type: SIGNALING_TYPE_CODE_TERMINAL_AI_ASSISTANT_UPDATED,
...
function renderAssistant(connectionId: string | null = 'conn-1') {
    ...
    const view = renderHook(() =>
        useTerminalAiAssistant({ connectionId, subscribe, sendMessage }),
    );
...
describe('useTerminalAiAssistant', () => {
    it('ask sends a TerminalAiAssistantAsk to the target and enters running', () => {
        ...
        expect(type).toBe(SIGNALING_TYPE_CODE_ASK_TERMINAL_AI_ASSISTANT);
    ...
    it('aggregates tool_started then final into a structured answer', () => {
        const { result, feed } = renderAssistant();
    ...
    it('replays prior completed turns as history on a follow-up ask', () => {
        ...
        const asks = sendMessage.mock.calls.filter(
            (c) => c[0] === SIGNALING_TYPE_CODE_ASK_TERMINAL_AI_ASSISTANT,
        );
    ...
    it('reset sends a cancel for an in-flight ask and clears state', () => {
        ...
        const cancel = sendMessage.mock.calls.find(
            (c) => c[0] === SIGNALING_TYPE_CODE_CANCEL_TERMINAL_AI_ASSISTANT,
        );
