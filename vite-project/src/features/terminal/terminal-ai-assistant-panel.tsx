...
import type {
    ...
    AssistantState,
    RiskLevel,
    TerminalAiAssistantMode,
} from './use-terminal-ai-assistant';
 ...
 *  `useConfirmExec`. Omitted when the assistant is rendered suggest-only. */
export type AssistantExecControls = {
 ...
 *  many commands. */
const AI_ASSISTANT_INDEX_STRIDE = 100;
const AI_ASSISTANT_MIN_WIDTH = 280;
const AI_ASSISTANT_MAX_WIDTH = 720;
        ...
        <div className="rounded-md border border-border/60 bg-background/40 p-2 text-sm">
            <div className="flex items-start justify-between gap-2">
                ...
                <Badge variant="outline" className={riskClass(suggestion.risk)}>
                    {t(`pages.deskTerminal.assistant.risk.${suggestion.risk}`, suggestion.risk)}
                </Badge>
            </div>
            {suggestion.note && (
            ...
            {blocked ? (
                <div className="mt-2 flex items-center gap-1 text-xs text-red-400">
                    ...
                    {t(
                        'pages.deskTerminal.assistant.blockedHint',
                    )}
                </div>
            ) : (
                <>
                    <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        >
                            ...
                            {t('pages.deskTerminal.assistant.fill')}
                        </Button>
                        <Button size="sm" variant="ghost" onClick={copy}>
                            <ClipboardCopy className="mr-1 h-3.5 w-3.5" />
                            {copied
                                ? t('pages.deskTerminal.assistant.copied')
                                : t('pages.deskTerminal.assistant.copy')}
                        </Button>
                        {canRun && !entry && (
                            <Button
                                size="sm"
                            ...
                            >
                                ...
                                {t('pages.deskTerminal.assistant.run')}
                            </Button>
                        )}
                        {canRun && !entry && (
                            <Button
                            ...
                            >
                                ...
                                {t('pages.deskTerminal.assistant.runInteractive')}
                            </Button>
                        )}
                    </div>
                    {exec && entry && (
                    ...
                    {modeBlocked && (
                        <p className="mt-1 text-[10px] text-muted-foreground">
                            {t('pages.deskTerminal.assistant.execGuide')}
                        </p>
                    )}
                </>
            )}
...
export type TerminalAiAssistantPanelProps = {
    ...
    onAsk: (mode: TerminalAiAssistantMode, question: string, modelId: number | null) => void;
    onReset: () => void;
    onClose: () => void;
     ...
     *  assistant catalog and preference are org-scoped. Undefined for the personal
     *  view / open-source control end. */
    orgId?: number;
};

/**
 * The in-Terminal AI Assistant side panel. Two scenarios share one surface:
 ...
 */
export function TerminalAiAssistantPanel({
    state,
    onAsk,
    onReset,
    onClose,
    ...
    exec,
    orgId,
}: TerminalAiAssistantPanelProps) {
    const { t } = useTranslation();
    const [mode, setMode] = useState<TerminalAiAssistantMode>('how_to');
    ...
    const onResizePointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        setPanelWidth(
            Math.min(
                ...
                containerLimit,
                Math.max(AI_ASSISTANT_MIN_WIDTH, start.width + start.x - event.clientX),
            ),
        );
    };
    ...
    return (
        ...
        >
            <div
                ...
                aria-label={t('pages.deskTerminal.assistant.resizePanel')}
            ...
            <div className="relative min-h-0 flex-1">
                <div
                    ref={scrollRef}
                    onScroll={onScroll}
                    data-testid="terminal-ai-assistant-scroll-area"
                ...
                >
                    {t('pages.deskTerminal.assistant.aiIdentityNotice')}
                </p>
                        ...
                        <div key={turnIndex} className="space-y-2">
                            <div className="flex justify-end">
                                <div className="max-w-[90%] whitespace-pre-wrap break-words rounded-md bg-primary/10 px-2 py-1 text-sm text-foreground">
                                    {turn.question ||
                                        t('pages.deskTerminal.assistant.explainErrorTurn')}
                                </div>
                            </div>
                            ...
                            {turn.answer ? (
                                <div className="space-y-3">
                                    {/* AI-generated marking (Art.50(2)) for the
                                        assistant answer. Driven by the answer being
                                    ...
                                    {turn.answer.suggestions.map((s, rowIndex) => (
                                        <SuggestionRow
                                            key={rowIndex}
                                            index={turnIndex * AI_ASSISTANT_INDEX_STRIDE + rowIndex}
                                            suggestion={s}
                                            onFill={onFill}
                                            exec={exec}
                                        />
                                    ...
                                    {turn.answer.suggestions.length === 0 && (
                                        <p className="text-xs text-muted-foreground">
                                            {t('pages.deskTerminal.assistant.noSuggestions')}
                                        </p>
                                    )}
                                </div>
                            ) : (
                                            ...
                                            <div className="space-y-1">
                                                {state.tools.map((tool, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        {t(
                                                            'pages.deskTerminal.assistant.toolRan',
                                                            { name: tool.name },
                                                        )}
                                                    </div>
                                                ))}
                                        ...
                                        {running &&
                                            ...
                                            state.tools.length === 0 && (
                                                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                                    ...
                                                    {t('pages.deskTerminal.assistant.thinking')}
                                                </div>
                                            )}
                                                ...
                                                <span>
                                                    {state.retractionReason
                                                        ...
                                                        : agentErrorMessage(
                                                              ...
                                                              state.errorCode,
                                                              state.error,
                                                              t('pages.deskTerminal.assistant.error'),
                                                          )}
                                                </span>
                                            </div>
                                        )}
                ...
                {showJumpToLatest && (
                    <Button
                        ...
                        aria-label={t('pages.deskTerminal.assistant.scrollToLatest')}
                        title={t('pages.deskTerminal.assistant.scrollToLatest')}
                    >
            ...
            <div className="border-t border-border p-3">
                ...
                <p className="mb-2 text-xs text-muted-foreground">
                    {t('pages.deskTerminal.assistant.aiDisclaimer')}
                </p>
                <div className="mb-2 flex gap-1">
                    ...
                    >
                        {t('pages.deskTerminal.assistant.modeHowTo')}
                    </Button>
                    <Button
                        size="sm"
                    ...
                    >
                        {t('pages.deskTerminal.assistant.modeExplain')}
                    </Button>
                </div>
                {mode === 'how_to' ? (
                    <Textarea
                        ...
                        placeholder={t(
                            'pages.deskTerminal.assistant.askPlaceholder',
                        )}
                        value={question}
                    ...
                    <p className="text-xs text-muted-foreground">
                        {t(
                            'pages.deskTerminal.assistant.explainHint',
                        )}
                    </p>
                )}
                ...
                <div className="mt-2 flex gap-2">
                    <Button size="sm" onClick={submit} disabled={running}>
                        ...
                        {t('pages.deskTerminal.assistant.ask')}
                    </Button>
                    {(state.phase === 'done' || state.phase === 'error' || running) && (
                        <Button size="sm" variant="ghost" onClick={onReset}>
                            {t('pages.deskTerminal.assistant.reset')}
                        </Button>
                    )}
                </div>
            </div>
