...
export function ScheduleProposalCards({ tools, running = false, deviceId, connectionId }: { tools: DeviceAssistantToolActivity[]; running?: boolean; deviceId: string; connectionId: string }) {
    ...
    useEffect(() => {
        ...
        for (const tool of tools) {
            ...
            try {
                ...
                if (value.state === 'draft' && ['fresh_task', 'conversation_resume'].includes(value.kind)
                    ...
                    if (value.kind === 'conversation_resume' && value.awaiting_confirmation === true) waiting.current.add(value.schedule_id);
                }
        ...
        if (!review && queued.current.length) {
            ...
            if (index >= 0) setReview(queued.current.splice(index, 1)[0]);
        }
            ...
            <DialogContent className="flex max-h-[85vh] flex-col overflow-hidden sm:max-w-xl">
                ...
                <div className="min-h-0 overflow-y-auto">
                    {review && <ProposalReview client={client} scheduleId={review} connected={isConnected} activationDisabled={running && !waiting.current.has(review)}
                        zone={Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC'} assistantPaths={{ [deviceId]: `/desk/${encodeURIComponent(connectionId)}/assistant` }} approvalDialog onChanged={task => { setDecisions(previous => ({ ...previous, [task.schedule_id]: task.status })); setReview(null); }} />}
                </div>
            </DialogContent>
        </Dialog>
    </>;
