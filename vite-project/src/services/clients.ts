 ...
 */
export async function testModelProvider(config: Partial<RequestConfig> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config



  const res = await request<TestModelProviderMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "POST", url : getTestModelProviderUrl().url.toString(), ... requestConfig })
  return res.data
}
...
export async function getTurnSession(params: GetTurnSessionQueryParams, config: Partial<RequestConfig> & { client?: Client } = {}) {
  ...
  const res = await request<GetTurnSessionQueryResponse, ResponseErrorConfig<GetTurnSession404>, unknown>({ method : "GET", url : getGetTurnSessionUrl().url.toString(), params, ... requestConfig })
  return res.data
}
