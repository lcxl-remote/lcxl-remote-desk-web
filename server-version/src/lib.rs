...
/// (`FleetExecRequest` / `FleetExecResult`), so the manager can dispatch a
/// sealed `ExecPlan` to it. See [`FLEET_EXEC_MIN_SERVER_API_VERSION`].
...
pub const FLEET_EXEC_MIN_SERVER_API_VERSION: i32 = 1;
