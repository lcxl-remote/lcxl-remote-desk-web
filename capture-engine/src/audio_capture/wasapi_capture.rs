...
pub fn init_thread() -> Result<(), CaptureError> {
    unsafe { CoInitializeEx(None, COINIT_APARTMENTTHREADED).ok()? };
    Ok(())
}

pub fn destroy_thread() -> Result<(), CaptureError> {
...
impl WasapiAudioCapture {
    pub fn get_device_id(device: &IMMDevice) -> Result<String, CaptureError> {
    ...
    pub fn get_buffer_size(&self) -> Result<u32, CaptureError> {
        let packet_size = unsafe { self.audio_capture_client.GetNextPacketSize() }?;
        Ok(packet_size)
    }

    pub fn get_one_buffer(&self) -> Result<WasapiAudioBuffer, CaptureError> {
...
mod tests {
    ...
    #[test]
    fn test_device_info() -> Result<(), CaptureError> {
        initialize();
