...
impl PortalClient<'_> {
    ...
    pub fn select_sources(&self, session: &PortalSession) -> Result<(), CaptureError> {
        let conn = get_zbus_connection()?;
        let mut options = HashMap::new();
        let handle_token = rand::random::<u32>().to_string();
        let request = get_zbus_portal_request(conn, &handle_token)?;
    ...
    pub fn start(&self, session: &PortalSession) -> Result<ScreenCastStartResponse, CaptureError> {
        let conn = get_zbus_connection()?;
        let mut options = HashMap::new();
        let handle_token = rand::random::<u32>().to_string();
        let request = get_zbus_portal_request(conn, &handle_token)?;
    ...
    pub fn open_pipewire_remote(&self, session: &PortalSession) -> Result<OwnedFd, CaptureError> {
