//! Browser-facing access to an OSS AI Assistant conversation.
//!
...
#[utoipa::path(
    ...
    responses((status = 200, description = "Conversation snapshot, or a uniform \
        not-found/not-accessible response", body = RestResponse<AiAssistantSessionSnapshotDto>)),
)]
#[get("/my/ai-assistant-session")]
pub async fn get_ai_assistant_session(
    connection_map: web::Data<SharedConnectionMap>,
    query: web::Query<AiAssistantSessionQuery>,
) -> Result<HttpResponse, DeskSignalError> {
    let actor_id = SINGLE_ACCOUNT_USER_ID.to_string();
...
#[post("/my/ai-assistant-session/capability-grant/revoke")]
pub async fn revoke_ai_assistant_capability_grant(
    connection_map: web::Data<SharedConnectionMap>,
...
#[post("/my/ai-assistant-session/permission-decision")]
pub async fn decide_ai_assistant_permission(
    connection_map: web::Data<SharedConnectionMap>,
...
pub(crate) async fn decide_permission_on(
    ...
    let registry =
        ai_assistant_provider_registry().with_web_search_binding(search_config.binding());
    ...
    let inventory = project_capability_availability(
        ...
        now_unix_ms,
        crate::ai_assistant_orchestrator::oss_central_capability_readiness(
...
#[post("/my/ai-assistant-session/background-task/cancel")]
pub async fn cancel_ai_assistant_background_task(
    connection_map: web::Data<SharedConnectionMap>,
...
#[utoipa::path(
    ...
    responses((status = 200, description = "Authorized recent AI Assistant sessions",
        body = RestResponse<AiAssistantSessionListDto>)),
)]
#[get("/my/ai-assistant-sessions")]
pub async fn list_ai_assistant_sessions(
    connection_map: web::Data<SharedConnectionMap>,
    query: web::Query<AiAssistantSessionListQuery>,
) -> Result<HttpResponse, DeskSignalError> {
    let target_audience = {
        let map = connection_map.read().await;
    ...
    let summaries = SignalAgentSessionStore::new(crate::db::get_db().clone())
        .list_ai_assistant_sessions(&actor_id, &target_audience, limit)
        .await
        .map_err(|error| {
            DeskSignalError::new_custom_error(DeskErrorCode::SYSTEM_ERROR, &error.message)
        })?;
    let sessions = summaries
        .into_iter()
        .map(|summary| AiAssistantSessionSummaryDto {
        ...
        .collect();
    Ok(
        HttpResponse::Ok().json(RestResponse::succeed_with_data(AiAssistantSessionListDto {
            sessions,
        })),
    )
}

#[utoipa::path(tag = TAG, summary = "Delete a AI Assistant conversation and stop its AI turn",
    request_body = DeleteAiAssistantSessionBody,
    responses((status = 200, body = RestResponse<DeleteAiAssistantSessionResponse>)))]
#[post("/my/ai-assistant-session/delete")]
pub async fn delete_ai_assistant_session(
    connection_map: web::Data<SharedConnectionMap>,
    query: web::Json<DeleteAiAssistantSessionBody>,
) -> Result<HttpResponse, DeskSignalError> {
    use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};
    let actor_id = SINGLE_ACCOUNT_USER_ID.to_string();
