...
impl ScheduleStore {
    ...
    pub async fn supersede_stale_continuation(
        &self,
        owner: i32,
        run_id: &str,
        ...
        let task = entity::Entity::find()
            ...
            .await?
            .ok_or(ScheduleStoreError::NotFound)?;
        if task.kind != "conversation_resume" || task.active_run_id.as_deref() != Some(run_id) {
            return Ok(false);
        }
        ...
        let source = agent_session::Entity::find()
            .filter(agent_session::Column::ConversationId.eq(&work.conversation_id))
            .filter(agent_session::Column::ActorId.eq(owner.to_string()))
            .filter(agent_session::Column::DeviceId.eq(&task.target_device_id))
            .lock_exclusive()
            .one(&txn)
            .await?
            .ok_or(ScheduleStoreError::NotFound)?;
        let session = PersistedAgentSession::decode_json(&source.state_json)
            .map_err(|_| ScheduleStoreError::Invalid)?;
        if source.version != session.version
            || session.conversation_id != work.conversation_id
            || task.source_conversation_id.as_deref() != Some(work.conversation_id.as_str())
            || session
                .check_subject(&owner.to_string(), &task.target_device_id)
                .is_err()
            || session
        ...
        if i64::try_from(session.input_revision).map_err(|_| ScheduleStoreError::Invalid)?
            <= required
        {
            return Ok(false);
        }
        ...
        settle(Settlement {
            ...
            work,
            now,
            outcome: ScheduledRunStatus::Superseded,
