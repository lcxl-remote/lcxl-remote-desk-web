...
//! mints a grant here, and the RequestRemoteAccess authorizer looks it up when stamping
...
/// lookup-and-stamp (RequestRemoteAccess authorizer) must resolve to the same instance.
...
/// process, e.g. a pure desk-server mode). Mirrors the RequestRemoteAccess authorizer's
